...
static bool cli_shell_line_input_del(CliKeyCombo combo, void* context) {
    UNUSED(combo);
    CliShellLine* line = context;
    // erase one character forward, at the cursor, without moving it — the
    // mirror image of bksp above, which erases behind the cursor and moves
    // it back by one.
    cli_shell_line_ensure_not_overwriting_history(line);
    FuriString* editing_line = cli_shell_line_get_editing(line);
    cli_shell_line_clamp_position(line, editing_line);
    ...
    // print the rest of the line, restore cursor (no initial left-shift:
    // the cursor doesn't move for a forward delete)
    printf("%s" ANSI_ERASE_LINE(ANSI_ERASE_FROM_CURSOR_TO_END), furi_string_get_cstr(editing_line) + line->line_position);
    size_t left_by = furi_string_size(editing_line) - line->line_position;
    if(left_by) printf(ANSI_CURSOR_LEFT_BY("%zu"), left_by);
    stdio_flush();
    return true;
}
    ...
    .records =
        {
            ...
            {{CliModKeyNo, CliKeyDelete}, cli_shell_line_input_del},
